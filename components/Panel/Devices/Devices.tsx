...
const Devices = () => {
  ...
  let [device, setDevice] = useState<DevicesInterface>(devices);
  ...
  if (isLoadingDevices) return <div>Loading</div>;
    ...
    <>
      <div className="flex flex-col text-white bg-gradient-to-r from-gray-800 via-gray-700 to-gray-600 rounded-xl">
        <div className="flex mt-4 ml-4 ">
          ...
          <span className="items-center my-auto ml-2 text-xl font-bold">
            Dostupni uređaji
          </span>
        </div>
          ...
          <div className="inline-block min-w-full py-2 sm:px-6 lg:px-8">
            <div className="overflow-hidden">
              <table className="min-w-full text-sm font-light text-left">
                ...
                <tbody>
                  {devices &&
                    devices.map((device: DevicesInterface) => (
                      <tr
                        className="transition duration-300 ease-in-out border-b hover:bg-gradient-to-r hover:from-gray-700 hover:via-gray-600 hover:to-gray-500"
                        key={device.id}
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <button className="flex items-center w-12 h-6 transition duration-300 bg-white rounded-full shadow focus:outline-none">
                            <div
                              id="switch-toggle"
                              onClick={() => {
                                handleSwitcher(device.id as number);
                              }}
                              className={
                                device.status
                        ...
                        <td className="px-6 py-4 space-x-2 whitespace-nowrap">
                          <button
                            type="button"
                            className="px-4 py-2 font-semibold text-white duration-200 ease-in bg-blue-500 rounded hover:bg-blue-600"
                            onClick={handleModal}
                          >
                            Kontrola
                          </button>
                          <button
                            type="button"
                            className="px-4 py-2 font-semibold text-white duration-200 ease-in bg-red-500 rounded hover:bg-red-600"
                          >
                            Izbriši
                          </button>
                        </td>
            ...
            <div className="relative w-full h-full xs:max-w-xs sm:max-w-md md:max-w-lg lg:max-w-2xl xl:max-w-4xl xl:-right-[5rem] md:h-auto right-[2%]">
              <div className="relative bg-white rounded-lg shadow dark:bg-gray-800">
                <button
                  type="button"
                  onClick={handleModal}
                  className="absolute top-3 shadow-xl right-2.5 duration-500 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white"
                ...
                <div className="px-6 py-6 lg:px-8">
                  <h3 className="flex items-center mb-4 text-2xl font-medium text-gray-900 dark:text-white">
                    <MdCreateNewFolder className="p-1 mr-2 rounded-full bg-gradient-to-r from-primaryColor to-secondaryColor w-9 h-9" />
                    Kontrola uređaja
                  </h3>
                  <form className="space-y-6" action="#">
                    <div>
                      ...
                      >
                        ...
                        {rooms.map((room, index) => {
                          return (
                            <option key={index} value={room.prostorija}>
                              {room.prostorija}
                            </option>
                          );
                        })}
                    ...
                    <button
                      type="submit"
                      className="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-secondaryColor dark:hover:bg-primaryColor duration-500"
                      onClick={AddRoom}
                    >
                      Kreiraj
                    </button>
